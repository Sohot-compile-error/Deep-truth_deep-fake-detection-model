...
def eval_func(
   ...
   # assert real_dir.is_dir()
   # assert fake_dir.is_dir()
    daehwa_dir = "/home/jms/audio"
    # melganLarge_dir = fake_dir / "ljspeech_melgan_large"
   # assert melgan_dir.is_dir()
    # assert melganLarge_dir.is_dir()

    daehwa_dir = '/home/jms/audio'
    _, daehwa_dataset = load_directory_split_train_test(
        path=daehwa_dir,
        feature_fn=feature_fn,
        feature_kwargs={},
        test_size=test_size,
    ...
    model = Model(**model_kwargs).to(device)
    # model_path = 'saved/SimpleLSTM_lfcc_O/best.pt'
